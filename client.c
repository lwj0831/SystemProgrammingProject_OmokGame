...
int inputUserName(char*);
void playOmok(user*, user*, int);
int gameEnd(void);
void manual(void);
...
char* board[18][18];
user user1; // 서버의 이름을 저장
user user2; // 클라이언트의 이름을 저장
...
int main(void) {
    ...
    while (1) {
        ...
        // choose game start menu
            ...
            result = inputUserName(user2.name);
            ...
            // 서버에서 user1 이름 수신
            read(sock, user1.name, sizeof(user1.name));
            // 클라이언트 이름 전송
            send(sock, user2.name, strlen(user2.name), 0);
            ...
            // 클라이언트 playOmok 실행
            playOmok(&user1, &user2, sock);
            user_arr[len++] = user1; // add user1
            user_arr[len++] = user2; // add user2
            // user_arr[]->"score.txt" (update the result of omok game and userInfo(win/lose))
            FILE* fp2 = NULL;
...
int inputUserName(char* user2_name) {
    initscr();
    cbreak();
    noecho();
    keypad(stdscr, TRUE);
    ...
    while (1) {
        ...
        // Copy the cleaned name to user2.name
        strncpy(user2_name, temp_name, sizeof(user2.name) - 1);
        user2_name[sizeof(user2.name) - 1] = '\0';

        if (strlen(user2_name) == 0) {
...
void playOmok(user* user1, user* user2, int sock) {
    initscr();
    clear();
    cbreak();
    noecho();
    ...
    while (1) {
        ...
        if (count % 2 == 0) {
            ...
            int bytes_read = read(sock, buf, 2000);
        ...
        // 백돌 차례(클라이언트)
            ...
            while (1) {
                ...
                if (key == 27) { // Press Esc Key
                    ...
                    endwin();
                    close(sock);
                    printf("Game terminated by client.\n");
                    return;
                }
            }
        }
